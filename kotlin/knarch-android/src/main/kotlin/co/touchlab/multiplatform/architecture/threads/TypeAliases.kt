...
actual class MediatorLiveData<T>actual constructor():android.arch.lifecycle.MediatorLiveData<T>(){
    actual fun <S> addSource(source:MutableLiveData<S>, onChanged:Observer<S>){
        super.addSource(source as android.arch.lifecycle.LiveData<S>, onChanged)
    }

    actual fun <S> removeSource(toRemote:MutableLiveData<S>){
        super.removeSource(toRemote as android.arch.lifecycle.LiveData<S>)
    }
}